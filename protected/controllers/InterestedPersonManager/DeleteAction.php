...
class DeleteAction extends CAction
{
    ...
    public function run($id, $type_lico, $id_yur, $type_yur, $date, $number_stake='')
    {
        ...
        $controller->pageTitle .= ' | Удаление менеджера';

        $org = Organization::model()->findByPk($id_yur, $controller->getForceCached());
        $model = InterestedPersonManager::model()->findByPk($id, $type_lico, $id_yur, $type_yur, $date, $number_stake, $controller->getForceCached());
            ...
            switch ($_POST['result']) {
                case 'yes':
                    if ($model->delete()) {
                        ...
                        throw new CHttpException(500, 'Не удалось удалить менеджера.');
                    }
                break;
                default:
