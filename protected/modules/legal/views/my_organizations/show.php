...
<div class="yur-tabs">
    <?php $this->widget('bootstrap.widgets.TbMenu', array(
        ...
        'items'=>array(
            array('label'=>'Информация', 'url'=>$this->createUrl('show', array('id' => $this->organization->id)), 'active'=> $this->cur_tab == 'info' ),
            array('label'=>'Документы',  'url'=>$this->createUrl('documents', array('id' => $this->organization->id)), 'active'=> $this->cur_tab == 'documents'),
            array('label'=>'Банковские счета', 'url'=>$this->createUrl('settlements', array('id' => $this->organization->id)), 'active'=> $this->cur_tab == 'settlements'),
            array(
                'label'=>'Заинтересованные лица',
                'url'=>$this->createUrl('benefits', array('id' => $this->organization->id)),
                'active'=> $this->cur_tab == 'benefits',
                'itemOptions' => array('class'=>'narrow')
            ),
            array(
                'label'=>'Календарь событий',
                'url'=>$this->createUrl('my_events', array('id' => $this->organization->id)),
                'active'=> $this->cur_tab == 'my_events',
                'itemOptions' => array('class'=>'narrow narrower')
            ),
            array('label'=>'Договоры', 'url'=>$this->createUrl('contracts', array('id' => $this->organization->id)), 'active'=> $this->cur_tab == 'contracts'),
